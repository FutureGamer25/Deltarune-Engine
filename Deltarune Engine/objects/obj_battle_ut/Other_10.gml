...
data_draw = function(){
	...
	draw_text(30,403,global.player_info.name + "   LV " + string(global.player_info.lv));
	#endregion
	
	#region HP
	draw_sprite(spr_hp_ut,0,244,407)
	var hp_max_var = global.player_info.hp_max;
	var hp_var = global.player_info.hp;
	var hp_max = (hp_max_var * 1.25);
	var hp = (hp_var * 1.25);
	draw_sprite_ext(spr_pixel,0,275,402,1*hp_max,21,0,#fe0000,1);
	draw_sprite_ext(spr_pixel,0,275,402,1*hp,21,0,#F3F93C,1);
	draw_text(289+(hp_max),403,string(hp_max_var)+" / "+string(hp_var));
	/*
